...
class ReservoirMatrix(RGobj):
    ...
    ``%``:
        ...
        (matrix indices jk, ij → ik with sum over j), i.e.
        |          ⎛ ⊤   ⊤⎞⊤
        |  A % B = ⎜A @ B ⎟
        |          ⎝      ⎠
        if there are no energy argument shifts.
    """
    ...
    def __rmul__(self, other):
        if isinstance(other, ReservoirMatrix):
            ...
            res = ReservoirMatrix(
                self.global_properties, other.symmetry * self.symmetry
            )
    ...
    def __matmul__(self, other):
        if isinstance(other, ReservoirMatrix):
            ...
            if symmetry == 0 or settings.IGNORE_SYMMETRIES:
                ...
                res[1, 1] = (
                    self[1, 1] @ other[1, 1] + symmetry * res_01_10.floquetConjugate()
                )
                res[1, 0] = self[1, 0] @ other[0, 0] + self[1, 1] @ other[1, 0]
            return res
        elif isinstance(other, RGfunction):
    ...
    def __mod__(self, other):
        ...
        if symmetry == 0 or settings.IGNORE_SYMMETRIES:
            ...
            res[1, 1] = (
                self[1, 1] @ other[1, 1] + symmetry * res_10_01.floquetConjugate()
            )
            res[1, 0] = self[0, 0] @ other[1, 0] + self[1, 0] @ other[1, 1]
        return res
    ...
    def __eq__(self, other):
        return self.global_properties is other.global_properties and np.all(
            self.data == other.data
        )
    ...
    def check_symmetry(self):
        ...
        if self.symmetry:
            assert np.allclose(
                self[0, 1].values, self.symmetry * self[1, 0].floquetConjugate().values
            )
...
def einsum_34_12_43_double(
    a: ReservoirMatrix, b: ReservoirMatrix, c: ReservoirMatrix, d: ReservoirMatrix
...
def product_combinations(
    a: ReservoirMatrix, b: ReservoirMatrix
) -> (ReservoirMatrix, ReservoirMatrix):
    """
    Equivalent to:
    >>> lambda a, b: a @ b, a % b
