    ...
    with tb.open_file(filename, "a") as h5f:
        ...
        for group in nodes:
            ...
            for var in ("gamma", "z", "deltaGamma"):
                node = group[var]
                if node.ndim == 3 and node.shape[0] % 2 and node.shape[1] == node.shape[2]:
                    vb = node.shape[0] // 2
                    arr = node[vb].copy()
            ...
            for var in ("g2", "g3"):
                node = group[var]
                if node.ndim == 5 and node.shape[0] == 2 and node.shape[1] == 2 and node.shape[2] % 2 and node.shape[3] == node.shape[4]:
                    vb = node.shape[2] // 2
                    arr = node[:,:,vb].copy()
                    h5f.remove_node(f"/data/{name}", var)
                    h5f.create_array(f"/data/{name}", var, arr)
            ...
            try:
                ...
                for key, value in metadata.items():
                    try:
                        ...
                        settings.logger.warning("failed to set metadata value:", exc_info=1)
                row.append()
...
class DataManager:
        ...
        try:
            ...
            with self.engine.begin() as connection:
                ...
                self.fourier_coef_table = db.Table(
                        ...
                        self.metadata,
                        db.Column('id', db.INTEGER(), primary_key=True),
                        db.Column('fcr0', db.FLOAT(), default=0.),
                        db.Column('fci0', db.FLOAT(), default=0.),
                        db.Column('fcr1', db.FLOAT(), default=0.),
                        db.Column('fci1', db.FLOAT(), default=0.),
                        db.Column('fcr2', db.FLOAT(), default=0.),
                        db.Column('fci2', db.FLOAT(), default=0.),
                        db.Column('fcr3', db.FLOAT(), default=0.),
                        db.Column('fci3', db.FLOAT(), default=0.),
                        db.Column('fcr4', db.FLOAT(), default=0.),
                        db.Column('fci4', db.FLOAT(), default=0.),
                        db.Column('fcr5', db.FLOAT(), default=0.),
                        db.Column('fci5', db.FLOAT(), default=0.),
                        db.Column('fcr6', db.FLOAT(), default=0.),
                        db.Column('fci6', db.FLOAT(), default=0.),
                        db.Column('fcr7', db.FLOAT(), default=0.),
                        db.Column('fci7', db.FLOAT(), default=0.),
                        db.Column('fcr8', db.FLOAT(), default=0.),
                        db.Column('fci8', db.FLOAT(), default=0.),
                        db.Column('fcr9', db.FLOAT(), default=0.),
                        db.Column('fci9', db.FLOAT(), default=0.),
                    )
    ...
    def insert_from_h5file(self, filename):
        ...
        for dataset in KondoImport.read_all_from_h5(filename):
            ...
            if candidates.shape[0] > 0:
                settings.logger.debug(f"Found {candidates.shape[0]} times the same hash")
                exists = False
            ...
            if fourier_coef is not None:
                values = {'fcr%d'%i:f for i,f in enumerate(dataset.fourier_coef[:10].real)}
                values.update({'fci%d'%i:f for i,f in enumerate(dataset.fourier_coef[:10].imag)})
        ...
        try:
            ...
            pass
        settings.logger.info(f"Inserting {len(datasets)} new entries, ignoring {skipped}")
    ...
    def insert_in_db(self, filename : str, kondo : KondoExport):
        ...
        if fourier_coef is not None:
            ...
            values.update({f'fci{i}':float(f) for i,f in enumerate(fourier_coef[:10].imag)})
            stmt = self.fourier_coef_table.insert(values=values)
            with self.engine.begin() as connection:
                result = connection.execute(stmt)
                connection.commit()
    ...
    def list(self, min_version=(14,0,-1,-1), has_fourier_coef=False, **parameters):
        ...
        if fourier_coef is not None:
            if len(fourier_coef) < 10:
                fourier_coef = (*fourier_coef, *(0 for i in range(10-len(fourier_coef))))
            else:
                fourier_coef = fourier_coef[:10]
            preselected_table = self.df_table.loc[selection]
            with self.engine.begin() as connection:
                df_fourier_coef_table = pd.read_sql_table("fourier_coef", connection, index_col="id")
            fourier_coef_table = df_fourier_coef_table.loc[preselected_table.fourier_coef_id]
            valid = np.ones(fourier_coef_table.shape[0], dtype=bool)
            for i,f in enumerate(fourier_coef):
                valid &= np.isclose(fourier_coef_table[f"fcr{i}"], f.real) & np.isclose(fourier_coef_table[f"fci{i}"], f.imag)
    ...
    def list_fourier_coef(self, **parameters):
        ...
        with self.engine.begin() as connection:
            fourier_coef_table = pd.read_sql_table("fourier_coef", connection, index_col="id")
    ...
    def load_all(self, fourier_coef=None, **parameters):
        ...
        if fourier_coef is not None:
            ...
            for i,f in enumerate(fourier_coef[:10]):
                valid &= np.isclose(fourier_coef_table["fcr%d"%i], f.real) & np.isclose(fourier_coef_table["fci%d"%i], f.imag)
    ...
    def save_pole(self, kid, khash, pole):
        stmt = self.table.update().where((self.table.c.id==kid) & (self.table.c.hash==khash)).values(pole=pole)
        with self.engine.begin() as connection:
            result = connection.execute(stmt)
            connection.commit()
