...
def get_ref_nmax(dm, omega, vdc, vac, preset="precise", dc_shift=0, ac_shift=0, nmax_shift=0):
    parameters = dict(
            ...
            omega5 = dict(omega=16.5372, solver_tol_rel=1e-8, solver_tol_abs=1e-10, d=1e9, method="mu", padding=0, good_flags=0x1000, bad_flags=0x0ffc, xL=0.5, voltage_branches=4),
            )
        ...
        for key, value in options.items():
            ...
            if len(value) == 1:
                ...
                if scale[0] in ("lin", "linear"):
                    ...
                    raise ValueError("Unexpected value for parameters \"scale\": %s"%scale[0])
            elif len(value) == 3:
                dim = round(value[2])
                ...
                if scale[dim] in ("lin", "linear"):
                    ...
                    raise ValueError("Unexpected value for parameters \"scale\": %s"%scale[dim])
            else:
...
""",
    ...
    save_group.add_argument("--find_pole", type=int, default=0, choices=(0,1),
            help="Find and save pole of Γ", metavar="int")
    ...
    phys_group.add_argument("--fourier_phases", metavar='tuple', type=float, nargs='*',
    ...
    method_group.add_argument("--preset", metavar="str", type=str,
            choices=("normal", "precise", "old", "omega5"), default="precise",
    ...
    method_group.add_argument("--preset_correction", metavar="int", type=int, default=0,
    ...
    numerics_group.add_argument("--compact", metavar='{0,1,2}', type=int, nargs='+', default=0,
    ...
    if fourier_phases is not None:
        ...
        options["fourier_coef"] = tuple(c*np.exp(1j*np.pi*p) for c,p in zip(fourier_coef, fourier_phases))
    ...
    if threads == 1:
        ...
        for kondo_options in gen_option_iter(**options):
            ...
            settings.logger.info(f"Saving Vdc={kondo_options['vdc']:.8g}, Vac={kondo_options['vac']:.8g}, Ω={kondo_options['omega']:.8g} to {filename}")
            kid, khash = dm.save_h5(kondo, filename, include)
            if find_pole:
                pole, solver = kondo.findPole(**solver_options)
                settings.logger.info(f"Saving pole for Vdc={kondo_options['vdc']:.8g}, Vac={kondo_options['vac']:.8g}, Ω={kondo_options['omega']:.8g} to database")
