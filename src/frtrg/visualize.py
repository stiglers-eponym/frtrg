...
def main(dm, **parameters):
    for name in ("vdc", "vac", "omega"):
        if parameters.get(name, None) is not None:
            ...
            return 0
    print(
        """Invalid input: Please provide one of the parameters omega, vdc, and vac.
...
def parse():
    ...
    parser = argparse.ArgumentParser(
        description="Generate 3d overview plots using pyqtgraph"
    )
    parser.add_argument(
        "--db_filename",
        metavar="file",
    ...
    parser.add_argument(
        "--method", type=str, choices=("J", "mu"), help="method: J or mu"
    )
    parser.add_argument("--nmax", metavar="int", type=int, help="Floquet matrix size")
    parser.add_argument(
        "--padding", metavar="int", type=int, help="Floquet matrix ppadding"
    )
    parser.add_argument(
        "--voltage_branches", metavar="int", type=int, help="Voltage branches"
    )
    parser.add_argument(
        "--resonant_dc_shift", metavar="int", type=int, help="resonant DC shift"
    )
    parser.add_argument("--vdc", metavar="float", type=float, help="Vdc, units of Tkrg")
    fourier_coef_group = parser.add_mutually_exclusive_group()
    fourier_coef_group.add_argument(
        "--vac", metavar="float", type=float, help="Vac, units of Tkrg"
    )
    fourier_coef_group.add_argument(
        "--fourier_coef",
        metavar="tuple",
    ...
    parser.add_argument(
        "--d", metavar="float", type=float, help="D (UV cutoff), units of Tkrg"
    )
    parser.add_argument(
        "--xL", metavar="float", type=float, default=0.5, help="Asymmetry, 0 < xL < 1"
    )
    parser.add_argument(
        "--truncation_order",
        metavar="int",
    ...
    parser.add_argument("--grid", metavar="bool", type=int, default=0, help="Show grid")
    parser.add_argument(
        "--size",
        metavar="float",
        type=float,
    ...
    parser.add_argument(
        "--include_Ga", metavar="bool", type=int, help="include Ga in RG equations"
    )
    parser.add_argument(
        "--min_version_major",
        metavar="int",
    ...
    parser.add_argument(
        "--good_flags", metavar="int", type=int, default=0, help="Required solver flags"
    )
    parser.add_argument(
        "--bad_flags", metavar="int", type=int, default=0, help="Forbidden solver flags"
    )
    args = parser.parse_args()

    options = args.__dict__
    options["min_version"] = (
        options.pop("min_version_major"),
        options.pop("min_version_minor"),
        -1,
        -1,
    )
